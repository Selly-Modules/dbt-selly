...
select
    ...
    {{ json_extract('table_alias', '_airbyte_data', ['conditionForUser'], ['conditionForUser']) }} as conditionforuser,
    {{ json_extract('table_alias', '_airbyte_data', ['supplierCommission'], ['supplierCommission']) }} as suppliercommission,
    {{ json_extract_scalar('_airbyte_data', ['isUnlimitedQuantity'], ['isUnlimitedQuantity']) }} as isunlimitedquantity,
    {{ json_extract_scalar('_airbyte_data', ['maximumLimitPerUser'], ['maximumLimitPerUser']) }} as maximumlimitperuser,
    {{ json_extract_array('_airbyte_data', ['milestonesStatistic'], ['milestonesStatistic']) }} as milestonesstatistic,
